...
class GettersTest(AbstractTestCase):
    ...
    def test_microsecond(self):
        ...
        d = DateTime(1234, 5, 6, 7, 8, 9, 101112)
    ...
    def test_float_timestamp(self):
        d = DateTime(1970, 1, 1, 0, 0, 0, 123456)
    ...
    def test_int_timestamp_accuracy(self):
        ...
        d = DateTime(3000, 10, 1, 12, 23, 10, 999999)
    ...
    def test_offset_no_dst(self):
        self.assertEqual(-14400, DateTime.create(2012, 6, 1, tz='America/Toronto').offset)

    def test_offset_for_gmt(self):
        self.assertEqual(0, DateTime.create(2012, 6, 1, tz='GMT').offset)

    def test_offset_hours_with_dst(self):
        self.assertEqual(-5, DateTime.create(2012, 1, 1, tz='America/Toronto').offset_hours)

    def test_offset_hours_no_dst(self):
        self.assertEqual(-4, DateTime.create(2012, 6, 1, tz='America/Toronto').offset_hours)

    def test_offset_hours_for_gmt(self):
        self.assertEqual(0, DateTime.create(2012, 6, 1, tz='GMT').offset_hours)

    def test_offset_hours_float(self):
        self.assertEqual(9.5, DateTime.create(2012, 6, 1, tz=9.5).offset_hours)

    def test_is_leap_year(self):
        self.assertTrue(DateTime.create(2012, 1, 1).is_leap_year())
        self.assertFalse(DateTime.create(2011, 1, 1).is_leap_year())

    def test_is_long_year(self):
        self.assertTrue(DateTime.create(2015, 1, 1).is_long_year())
        self.assertFalse(DateTime.create(2016, 1, 1).is_long_year())

    def test_week_of_month(self):
        self.assertEqual(5, DateTime.create(2012, 9, 30).week_of_month)
        self.assertEqual(4, DateTime.create(2012, 9, 28).week_of_month)
        self.assertEqual(3, DateTime.create(2012, 9, 20).week_of_month)
        self.assertEqual(2, DateTime.create(2012, 9, 8).week_of_month)
        self.assertEqual(1, DateTime.create(2012, 9, 1).week_of_month)

    def test_week_of_year_first_week(self):
        self.assertEqual(52, DateTime.create(2012, 1, 1).week_of_year)
        self.assertEqual(1, DateTime.create(2012, 1, 2).week_of_year)

    def test_week_of_year_last_week(self):
        self.assertEqual(52, DateTime.create(2012, 12, 30).week_of_year)
        self.assertEqual(1, DateTime.create(2012, 12, 31).week_of_year)

    def test_timezone(self):
        d = DateTime.create(2000, 1, 1, tz='America/Toronto')
        self.assertEqual('America/Toronto', d.timezone.name)

        d = DateTime.create(2000, 1, 1, tz=-5)
    ...
    def test_is_weekday(self):
        d = DateTime.now().next(pendulum.MONDAY)
    ...
    def test_date(self):
        dt = DateTime(2016, 10, 20, 10, 40, 34, 123456)
        d = dt.date()
