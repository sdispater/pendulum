...
def test_add_years_positive():
    assert pendulum.date.create(1975).add(years=1).year == 1976


def test_add_years_zero():
    assert pendulum.date.create(1975).add(years=0).year == 1975


def test_add_years_negative():
    assert pendulum.date.create(1975).add(years=-1).year == 1974


def test_add_months_positive():
    assert pendulum.date.create(1975, 12).add(months=1).month == 1


def test_add_months_zero():
    assert pendulum.date.create(1975, 12).add(months=0).month == 12

    
def test_add_months_negative():
    assert pendulum.date.create(1975, 12).add(months=-1).month == 11
